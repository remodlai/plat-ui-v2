...
export const DataSourceNode = ({ data, selected }: NodeProps) => {
  return (
    <div 
      className={cn(
        'p-3 rounded-xl w-64 backdrop-blur-sm bg-gradient-to-br from-emerald-950/90 to-teal-900/80 border border-emerald-800/60 text-white shadow-lg transition-all duration-200',
    ...
    >
      ...
      <div className="bg-emerald-950/50 rounded-lg p-2 text-xs space-y-2 border border-emerald-900/60">
        <div className="flex justify-between">
          ...
          <span className="font-medium text-emerald-100">{data.source}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-emerald-300">Documents</span>
          <span className="font-medium text-emerald-100">{data.documents}</span>
        </div>
        <div className="flex justify-between">
